...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 NPW at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zNPW send
enum ZerocoinSpendStatus {
    ZNPW_SPEND_OKAY = 0,                            // No error
    ZNPW_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNPW_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNPW_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNPW_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNPW_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNPW_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNPW_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNPW_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNPW_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZNPW_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNPW_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNPW_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNPW_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNPW_SPENT_USED_ZNPW = 14,                      // Coin has already been spend
    ZNPW_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZNpwAutoBackups(bool fEnabled)
    {
    ...
    /** zNPW reset */
    boost::signals2::signal<void()> NotifyzNPWReset;
};
