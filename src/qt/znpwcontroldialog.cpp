...
#include "ui_znpwcontroldialog.h"
...
std::list<std::string> ZNpwControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNpwControlDialog::listMints;

ZNpwControlDialog::ZNpwControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNpwControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNpwControlDialog::~ZNpwControlDialog()
{
    delete ui;
}

void ZNpwControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNpwControlDialog::updateList()
{
...
void ZNpwControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNpwControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNpw_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNpwControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNpwControlDialog::GetSelectedMints()
{
...
void ZNpwControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
