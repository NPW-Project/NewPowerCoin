...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NPW);
    ...
    if (!settings.contains("nAnonymizeNpwAmount"))
        settings.setValue("nAnonymizeNpwAmount", 1000);

    nAnonymizeNpwAmount = settings.value("nAnonymizeNpwAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNpwAmount"))
        SoftSetArg("-anonymizenpwamount", settings.value("nAnonymizeNpwAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in npw.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNpwAmount:
            return QVariant(nAnonymizeNpwAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNpwAmount:
            nAnonymizeNpwAmount = value.toInt();
            settings.setValue("nAnonymizeNpwAmount", nAnonymizeNpwAmount);
            emit anonymizeNpwAmountChanged(nAnonymizeNpwAmount);
            break;
        case CoinControlFeatures:
