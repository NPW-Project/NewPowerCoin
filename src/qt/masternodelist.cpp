...
void MasternodeList::StartAlias(std::string strAlias)
{
    ...
    BOOST_FOREACH (CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
        if (mne.getAlias() == strAlias) {
            ...
            // check masternode connection
            CService service = CService(mne.getIp());
            CNode* pnode = ConnectNode((CAddress)service, NULL, false);
            if (!pnode) {
                strError = "Could not connect to " + service.ToString();
...
void MasternodeList::StartAll(std::string strCommand)
{
    ...
    BOOST_FOREACH (CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
        ...
        if (!pnode) {
            nCountFailed++;            
            strError = "Could not connect to " + service.ToString();
            strFailedHtml += "<br>Failed to start masternode.<br>Error: " + strError;
            continue;
        }
        pnode->Release();
...
void MasternodeList::on_createButton_clicked()
{
